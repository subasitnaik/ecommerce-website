...
import { formatInrAmount } from "../currency";
...
const SingleStats = ({
  ...
  title: string;
  value: string;
  /** When true, prefix with ₹ (Indian Rupee). */
  isCurrency?: boolean;
}) => {
  return (
    <div className="bg-whiteSecondary flex w-[300px] h-28 shadow-custom-white border-1 border-white flex items-center justify-between px-4 hover:scale-105 hover:ease-linear hover:duration-200 cursor-pointer">
      <div>
        ...
        <p className="text-blackPrimary text-2xl font-bold">
          {isCurrency ? formatInrAmount(value) : value}
        </p>
      </div>
      <img
        src="/admin-assets/valueGraph.svg"
        alt="Value Graph"
        className="w-32"
      />
    </div>
